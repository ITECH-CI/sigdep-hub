...
JAR=$(ls target/ingestion-api-*.jar 2>/dev/null | grep -v '\.original$' | head -1)
if [[ -z "$JAR" || ! -f "$JAR" ]]; then
  echo "ERROR: no ingestion-api-*.jar in target/. Run 'mvn -DskipTests package' from sigdep-hub/ first."
  exit 1
fi
